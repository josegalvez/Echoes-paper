...
\begin{equation}
V_{\mathrm{scalar}}(x) = \left(1-\frac{2M}{r(x)}\right)\left[\frac{\ell(\ell+1)}{r(x)^2}+\frac{2M}{r(x)^3}\right].
\end{equation}
After rearranging the variables, the equation of motion of the spherical modes is now written in its traditional linear waveform. In Fig.~\ref{fig:Potential_BH}, we observe the growth of the potential barrier with the quantum number $\ell$. The potential wall does not vanish at $\ell=0$ due to the extra term proportional to $r^{-3}$ appearing after the coordinate change, which replaces the radial damping in the original Schwarzschild coordinates $(t,r)$. Such a term becomes subdominant for all $\ell\geq 1$. Intuitively, it is reasonable to expect that the modes with frequency above a given threshold can cross the barrier, while reflecting the lower frequency modes. 

Now we setup the scattering problem for one of the spherical modes $(\mathcal{U}_{20})$ with the following initial conditions
\begin{equation}
...
\end{equation}
which corresponds to an ingoing Gaussian wavepacket containing an infinite number of frequencies.
