...
\caption{\label{fig:echo_sigma_odd} Left panel: Amplitudes of the first four echoes of $\Psi^{\mathrm{odd}}_{20}$ as a function of $\sigma$ for $\ell=2$. Right panel: Relative amplitude of the first four echoes compared to the amplitude of the transient. The points represent the simulated double/single wall pairs used in our analysis, as in Fig.~\ref{fig:echo_sigma}. At small widths, the transient decreases faster than the amplitudes of the echoes as $\sigma$ becomes smaller.} 
\end{figure*}

\begin{figure}[t!]
\centering
\includegraphics[width=.45\textwidth]{figures/Odd_echo_w_t2.pdf}
\caption{\label{fig:echo_long} Asymptotic solution of an ingoing Gaussian pulse with $\sigma=16~r_g$ after its dispersion by a Morris-Thorne wormhole with a longer throat (with a length of 100~$r_g$). The amplitude of the transient is three orders of magnitude larger than the first echo, showing that the echoes do not have a generically large magnitude.}
\end{figure}
...
\textcolor{red}{The length of the throat for a Morris-Thorne wormhole is an additional parameter of the effective potential cavity. Thus, we simulated the scattering of an arbitrary Gaussian pulse with $\sigma=16~r_g$ by a Morris-Thorne wormhole with a larger throat ($100~r_g$, which is more than two times larger than the cavity used in the previous sections of this paper). In Fig.~\ref{fig:echo_long}, we show that the transient has a slightly larger magnitude consistent with a cavity made out of potential barriers reflective to lower frequencies. The shape of these potential barriers is not different from the walls depicted in Fig.~\ref{fig:evenodd_potentials}, therefore it is still possible to find a frequency ``sweet spot'' for each wall. Moreover, the amplitude of the echoes is three orders of magnitude smaller than the transient, which is coherent with the results shown in this manuscript.}
